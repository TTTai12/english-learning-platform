...
### 4. Kỹ thuật Dynamic Classes trong Tailwind CSS (Bản đồ màu sắc động)
*   **Vấn đề thực tế (Lỗi kinh điển của Fresher):** 
    *   Khi muốn thay đổi màu sắc Badge động dựa vào tên tag, ta thường viết code dạng cộng chuỗi: `className={`bg-${color}-500`}`.
    *   **Kết quả:** Màu sắc không hiển thị trên trình duyệt. 
    *   **Nguyên nhân:** Tailwind CSS là một công cụ JIT (Just-In-Time) compiler. Nó quét qua toàn bộ mã nguồn tĩnh để tìm kiếm các chuỗi class hoàn chỉnh (như `bg-red-500`) để đóng gói vào file CSS cuối cùng. Tailwind không chạy code Javascript, nên nó không hiểu được giá trị của biến `color` và sẽ bỏ qua không sinh ra class CSS tương ứng.
*   **Giải pháp (Safe Dynamic Class Lookup Map):**
    *   Định nghĩa một từ điển đối tượng (Object Map) chứa các class hoàn chỉnh tĩnh, có kiểu dữ liệu chặt chẽ (`Record<string, string>`):
        ```ts
        const tagColors: Record<string, string> = {
          'Từ vựng': 'bg-blue-100 text-blue-700 dark:bg-blue-900/40',
          'Phát âm': 'bg-purple-100 text-purple-700 dark:bg-purple-900/40',
        };
        ```
    *   Khi sử dụng trong JSX, ta truy xuất trực tiếp qua key:
        ```tsx
        className={`px-2 py-0.5 rounded-full ${tagColors[task.tag] ?? 'bg-muted'}`}
        ```
    *   *Lợi ích:* Đảm bảo Tailwind biên dịch đúng 100%, an toàn về kiểu dữ liệu (Type-safe) và dễ dàng quản lý hệ màu sắc ở một nơi duy nhất.
